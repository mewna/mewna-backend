...
public class InternalSnowflake {
    /*
     ...
     * elapsed since the {@link #epoch} given by users on {@link InternalSnowflake}
     ...
     */
    private final long epoch = 1518566400000L;
     ...
     */
    public InternalSnowflake(final long datacenterId, final long workerId) {
     ...
     *         sequence number
     */
    public long[] parseId(final long id) {
     ...
     */
    public String formatId(final long id) {
        final long[] arr = parseId(id);
        final String tmf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new Date(arr[0]));
     ...
     * @return a long value
     */
    private long diode(final long offset, final long length) {
        final int lb = (int) (64 - offset);
        final int rb = (int) (64 - (offset + length));
        return -1L << lb ^ -1L << rb;
    }
